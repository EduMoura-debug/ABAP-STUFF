...
MODULE user_command_0100 INPUT.
  ...
  CASE sy-ucomm.
    WHEN 'BACK'.
      LEAVE TO SCREEN 0.
    WHEN 'CANC' OR 'ENDE'.
      CALL METHOD g_menu_remessa->free.
      CALL METHOD g_menu_fatura->free.
      CALL METHOD g_menu_ordem->free.
      CALL METHOD g_menu_tree->free.
      CALL METHOD g_custom01->free.
      CALL METHOD g_custom02->free.
      CALL METHOD g_custom03->free.
      CALL METHOD g_custom04->free.
      LEAVE PROGRAM.
...
DATA: v_vbeln          LIKE vbak-vbeln,
      ...
      lt_conditions_in TYPE STANDARD TABLE OF BAPICOND WITH HEADER LINE,
      ls_conditions_in TYPE BAPICOND,
      lt_conditions_inx TYPE STANDARD TABLE OF BAPICONDX WITH HEADER LINE,
...
CLASS cl_remessa_receiver IMPLEMENTATION.
  ...
  METHOD handle_on_drag.
    ...
    aux_data->row_index = e_row-index.
    aux_data->status = wa_remessa-icon.
    e_dragdropobj->object = aux_data.
...
CLASS cl_fatura_receiver IMPLEMENTATION.
  ...
  METHOD handle_user_command.
    CASE e_ucomm.
      WHEN 'FATURA'.
        PERFORM zf_realizar_faturamento.
      WHEN OTHERS.
    ENDCASE.
  ENDMETHOD.
  ...
  METHOD handle_double_click.
     IF e_column = 'FATURA'.
      CLEAR wa_fatura.
      READ TABLE it_fatura INTO wa_fatura INDEX e_row.

      SET PARAMETER ID 'VF' FIELD wa_fatura-fatura.
...
*    CREATE OBJECT aux_data.
    ...
    ELSEIF aux_data->status NE '@08@'.
      MESSAGE: s000(zcm_efmoura) WITH 'Não Houve Saída da Mercadoria' DISPLAY LIKE 'E'.
    ELSE.
      IF aux_data->c_source_text = 'G_ALV_REMESSA' AND aux_data->c_target_text = 'G_ALV_FATURA'.
        PERFORM display_fatura USING aux_data->index.
      ENDIF.
    ENDIF.
  ENDMETHOD.
ENDCLASS.
...
*********ITEM*********
        ...
        item-target_val = wa_ordem-netwr.
        item-plant      = p_werks.
...
*******CONDITIONS*******
        ...
        ls_conditions_in-itm_number = lv_item_number.
        ls_conditions_in-applicatio = 'V'.
        ls_conditions_in-cond_type  = 'ZPB2'.
        ls_conditions_in-cond_value = wa_ordem-netwr / 10.
        ls_conditions_in-cond_unit  = wa_ordem-meins.
        APPEND ls_conditions_in TO lt_conditions_in.

        CLEAR: ls_conditions_inx.
        ls_conditions_inx-itm_number = lv_item_number.
...
FORM zf_criar_remessa.
  ...
  DATA: ls_remessa TYPE tp_remessa.
...
**** Success ****
          ...
          MODIFY it_remessa FROM wa_remessa TRANSPORTING remessa icon WHERE ordem = wa_remessa-ordem.
    ...
    IF wa_remessa-remessa IS NOT INITIAL.
      ...
      AT END OF ordem.
        ...
        IF sy-subrc = 0.
          ...
          wa_remessa-icon = '@08@'.
          MODIFY it_remessa FROM wa_remessa TRANSPORTING icon WHERE ordem = wa_remessa-ordem. "INDEX sy-tabix? "Bug provavel
          CALL METHOD g_menu_remessa->refresh_table_display.
...
*--------------------------------------*
  wa_fcat-col_pos = '7'.
  wa_fcat-fieldname = 'FATURA'.
  wa_fcat-tabname = 'IT_FATURA'.
  wa_fcat-coltext = 'Fatura'.
  wa_fcat-outputlen = 10.
  wa_fcat-hotspot = 'X'.
  APPEND wa_fcat TO it_fcat_04.
  CLEAR wa_fcat.
...
*&      --> AUX_DATA_>INDEX
*&---------------------------------------------------------------------*
FORM display_fatura USING p_index.

  DATA: ls_remessa TYPE tp_remessa,
        ls_fatura  TYPE tp_fatura.

  READ TABLE it_fatura INTO ls_fatura WITH KEY remessa = p_index.
  IF sy-subrc = 0.
    MESSAGE: s000(zcm_efmoura) WITH 'Remessa já exibida' DISPLAY LIKE 'E'.
  ELSE.
    LOOP AT it_remessa INTO ls_remessa WHERE remessa = p_index.

      ls_fatura-ordem   = ls_remessa-ordem.
      ls_fatura-remessa = p_index.
      ls_fatura-item    = ls_remessa-item.
      ls_fatura-matnr   = ls_remessa-matnr.
      ls_fatura-fkimg   = ls_remessa-fkimg.
      ls_fatura-meins   = ls_remessa-meins.
      APPEND ls_fatura TO it_fatura.

      DELETE it_fatura WHERE remessa IS INITIAL.
...
FORM zf_realizar_faturamento .
  ...
  LOOP AT it_fatura INTO wa_fatura.
    ...
    IF wa_fatura-fatura IS INITIAL.
      ...
      CLEAR wa_billing.
      wa_billing-salesorg   = '1000'.
      wa_billing-distr_chan = '01'.
      wa_billing-division   = '05'.
      wa_billing-doc_type   = 'ZSSD'.
      wa_billing-ref_doc_ca = 'J'.
      wa_billing-sold_to    = p_kunnr.
      wa_billing-ref_doc    = wa_fatura-remessa. "wa_fatura-ordem.
      wa_billing-ref_item   = wa_fatura-item.
*      wa_billing-doc_number = wa_fatura-ordem. "wa_fatura-remessa.
*      wa_billing-itm_number = wa_fatura-item.
*      wa_billing-price_date = sy-datum.
*      wa_billing-material   = wa_fatura-matnr.
*      wa_billing-plant      = p_werks.
      ...
      IF it_success[] IS NOT INITIAL.
        ...
        READ TABLE it_success INTO wa_success WITH KEY ref_doc = wa_fatura-remessa.
        CLEAR ls_fatura.
        ls_fatura-fatura = wa_success-bill_doc.
        MODIFY it_fatura FROM ls_fatura TRANSPORTING fatura WHERE remessa = wa_fatura-remessa.
        CALL METHOD g_menu_fatura->refresh_table_display.
